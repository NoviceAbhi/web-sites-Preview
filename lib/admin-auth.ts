...
export const getAllUsersWithPlans = (): UserRegistrationLog[] => {
  ...
  const allUsers = users.map((user: any) => {
    ...
    if ((user as any).email === 'vohrraoverseas@gmail.com') {
      hasActivePlan = true
      currentPlan = {
        id: `plan_special_${Date.now()}`,
    ...
    return {
      id: (existingLog as any)?.id || `reg_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      userId: (user as any).id,
      name: (user as any).name,
      email: (user as any).email,
      phone: (user as any).phone || 'N/A',
      registrationDate: (existingLog as any)?.registrationDate || new Date().toISOString(),
      loginCredentials: {
        email: (user as any).email,
        password: (user as any).password
      },
      lastLogin: (existingLog as any)?.lastLogin,
      status: (existingLog as any)?.status || 'active',
      hasActivePlan,
